...
export const FileUpload = ({ onChange }: { onChange?: (filesList: FileType[]) => void }) => {
							...
							filesList.map((fileItem, idx) => (
								...
								>
									...
									<div className="flex text-sm md:flex-row flex-col items-start md:items-center w-full mt-2 justify-between text-neutral-600 dark:text-neutral-400">
										<p>文件修改时间 {new Date(fileItem.file.lastModified).toLocaleDateString()}</p>
										{fileItem.status === "success" && <span>上传成功</span>}
										{fileItem.status === "uploading" && (
											<p>
												上传进度: {parseFloat((fileItem.progress * 100).toString()).toFixed(2)}%
											</p>
										)}
