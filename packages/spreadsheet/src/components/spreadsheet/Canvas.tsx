...
export const Canvas: React.FC<CanvasProps> = ({
	...
	const scrollConfig = useMemo(
		() => ({
			totalWidth:
				headerColsWidth.reduce((sum, prev) => sum + prev, 0) * zoomSize + config.scrollAreaPadding,
			totalHeight:
				headerRowsHeight.reduce((sum, prev) => sum + prev, 0) * zoomSize + config.scrollAreaPadding,
	...
	// 滚动 hooks
	const { scrollPosition, handleScrollbarDragStart, handleWheel } = useSheetScroll(scrollConfig)
	...
	const totalWidth = useMemo(
		() => headerColsWidth.reduce((sum, prev) => sum + prev, 0) * zoomSize - scrollPosition.x,
		[headerColsWidth, scrollPosition.x, zoomSize]
	)
	const totalHeight = useMemo(
		() => headerRowsHeight.reduce((sum, prev) => sum + prev, 0) * zoomSize - scrollPosition.y,
	...
	const handleGetClient = useCallback(
		<T extends React.MouseEvent<HTMLCanvasElement, MouseEvent>>(
			...
			callback: (rowIndex: number, colIndex: number, x: number, y: number) => void
		) => {
			const canvas = canvasRef.current
	...
	return (
		<>
			...
			{scrollConfig && scrollConfig.totalHeight > scrollConfig.viewportHeight && (
				<ScrollBar
					type="vertical"
					viewportSize={scrollConfig.viewportHeight}
					contentSize={scrollConfig?.totalHeight}
