...
export type CellInputActionsType = {
	...
	updateInputSize: (currentCell: CellData, options?: CellInputUpdateInputOptions) => void
}
...
>(({ onChange }, ref) => {
	...
	const handleMouseDown = useCallback(
		(e: React.MouseEvent) => {
			...
			const { cursorIndex } = getCursorPosByXY(x, y, canvasWidth, selectedCell, lines)
			...
			const handleMouseMove = (moveEvent: MouseEvent) => {
				...
				const { cursorIndex } = getCursorPosByXY(moveX, moveY, canvasWidth, selectedCell, lines)
				if (selectionAnchor.current !== null && cursorIndex !== selectionAnchor.current) {
	...
	const handleKeyDown = useCallback(
		(e: React.KeyboardEvent) => {
			...
			const copy = () => {
				e.preventDefault()
				e.stopPropagation()
				const selectedText = selectionText && value.slice(selectionText.start, selectionText.end)
				if (selectedText) {
			...
			if ((e.ctrlKey || e.metaKey) && key === "x") {
				copy()
				if (selectionText) {
					const content = value.slice(0, selectionText.start) + value.slice(selectionText.end)
			...
			if ((e.ctrlKey || e.metaKey) && key === "v") {
				...
				navigator.clipboard.readText().then((clipboardText) => {
					let newValue = value.slice(0, cursorIndex) + clipboardText + value.slice(cursorIndex)
					let newCursor = cursorIndex + clipboardText.length
					if (selectionText) {
						// 有选区，替换选中内容
						newValue =
							value.slice(0, selectionText.start) + clipboardText + value.slice(selectionText.end)
			...
			if (
				...
				newValue = value.slice(0, cursorIndex) + e.key + value.slice(cursorIndex)
				newCursor = cursorIndex + 1
				if (selectionText) {
					newValue = value.slice(0, selectionText.start) + e.key + value.slice(selectionText.end)
					setSelectionText(null)
			...
			if (e.key === "Backspace") {
				...
				e.stopPropagation()
				if (selectionText) {
					const newValue = value.slice(0, selectionText.start) + value.slice(selectionText.end)
					setSelectionText(null)
					...
					if (currentLine && currentLine.startIndex > 0 && currentLine.startIndex === cursorIndex) {
						cursorLine.current -= 1
			...
			if (e.key === "ArrowRight") {
				...
				e.stopPropagation()
				setSelectionText(null)
				updateCell(selectedCell, value, Math.min(value.length, cursorIndex + 1))
				return
			}
			if (e.key === "ArrowUp") {
				e.preventDefault()
	...
	const handleCellInputActions: CellInputActionsType = useMemo(() => {
		return {
			...
			updateInputSize(currentCell, options = {}) {
				updateCell(currentCell, currentCell.value, currentCell.value.length, options)
			}
		}
	...
	const drawInput = useCallback(() => {
		...
		if (selectionText) {
			...
			for (let i = 0; i < lines.length; i++) {
				if (
					...
					(i === lines.length - 1 || lines[i + 1].startIndex > selectionText.start)
				) {
					startLineIndex = i
				}
			...
			for (let i = startLineIndex; i <= endLineIndex; i++) {
				...
				if (textAlign === "center") {
					...
					x = canvasWidth - config.inputPadding - ctx.measureText(line.content).width
				}
				// 起始位置 画布高度一半 - 文本总高度一半 + 行高 * 行号
				const startY = canvasRef.current.height / 2 - totalTextHeight / 2 + i * lineHeightPX
				let startX = 0
				if (i === startLineIndex) {
					// 起始行
					const textBeforeStart = value.substring(line.startIndex, selectionText.start)
					startX += ctx.measureText(textBeforeStart).width
				}

				if (i === endLineIndex) {
					// 结束行
					const textBeforeEnd = value.substring(line.startIndex, selectionText.end)
		...
		const startY = canvasRef.current.height / 2 - totalTextHeight / 2 + lineHeightPX / 2
		// 绘制文本
		for (let i = 0; i < lines.length; i++) {
				...
				>
					...
					<div
						...
						style={{
							...
							display: selectionText ? "none" : isFocused.current ? "block" : "none"
						}}
					/>
				</div>
			</div>
