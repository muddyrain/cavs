...
export const useInput = ({
	...
	const { getFontSize, getTextAlign, getVerticalAlign, setFontStyle } = useTools()
	...
	const updateCursorPosition = useCallback(
		(selectedCell: CellData | null, lines: LineType[], cursorIndex: number, width: number) => {
			...
			if (textAlign === "left") {
				...
				left = canvasWidth - currentLineWidth - config.inputPadding + beforeTextWidth
			}
			...
			const top = lastHeight.current / 2 - totalTextHeight / 2 + _cursorLine * lineHeightPX
	...
	const updateInputSize = useCallback(
		(selectedCell: CellData | null, lines: LineType[]) => {
			...
			const maxLineWidth = Math.max(...lines.map((line) => ctx.measureText(line.content).width))
			const width = Math.ceil(Math.max(maxLineWidth + config.inputPadding * 2, minSize.width))
			lastWidth.current = width
			const fontSize = getFontSize(selectedCell)
			const lineHeightPT = fontSize + 4
			const lineHeightPX = (lineHeightPT * 4) / 3
			const height = Math.ceil(Math.max(lineHeightPX * lines.length, minSize.height))
		...
		[canvasRef, minSize.width, minSize.height, setFontStyle, config.inputPadding, getFontSize]
	)
	// 获取输入框高度 - 真实高度 不包含缩放
	...
	const getInputStyle = useCallback(
		(currentCell: CellData | null, options: CellInputUpdateInputOptions = {}) => {
			if (!currentCell)
				return {
					maxHeight: 0,
					maxWidth: 0,
	...
	const setInputStyle = useCallback(
		(
			...
			if (containerRef.current) {
				const { maxHeight, maxWidth, verticalAlign, cellWidth, cellHeight, textAlign, x, y } =
					getInputStyle(currentCell, options)
				const { width, height, maxLineWidth } = updateInputSize(currentCell, lines)
				let left = 0
				const top = y - 2
				if (textAlign === "right") {
					// 减的是左右的 padding
				...
				setInputStyleState({
					...
					minWidth: height > maxHeight ? updates.minWidth + 15 : updates.minWidth
				})
				setTimeout(() => {
	...
	const getCursorPosByXY = useCallback(
		(x: number, y: number, canvasWidth: number, selectedCell: CellData, lines: LineType[]) => {
			...
			// 根据文本对齐方式调整点击位置
			if (textAlign === "center") {
				const lineStartX = canvasWidth / 2 - lineWidth / 2 - config.inputPadding
	...
	const getLines = useCallback(
		(
			...
			const checkInputOverflow = (allLines: LineType[]) => {
				if (!options?.wrap) {
					...
					const maxLineWidth =
						Math.max(...allLines.map((item) => item.width)) + config.inputPadding * 2
			...
			if (!wrap) {
				...
				return options?.isCheckOverflow ? checkInputOverflow(allLines) : { lines: allLines }
			}
			...
			hardLines.forEach((hardLine, hardLineIndex) => {
				...
				if (hardLine.length === 0) {
					allLines.push(createLine("", currentStartIndex, lineIndex++, hardLineIndex))
				...
				while (remainingText.length > 0) {
					...
					allLines.push(createLine(lineText, currentLineStartIndex, lineIndex++, hardLineIndex))
					// 更新剩余文本和起始索引
		...
		[canvasRef, getValue, getCellWidthHeight, config.inputPadding, getInputStyle]
	)
