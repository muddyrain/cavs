...
export type SheetStoreActionType = {
	...
	setCutSelection: React.Dispatch<React.SetStateAction<SelectionSheetType | null>>
...
export const SpreadsheetContext = React.createContext<
	| ({
			...
			updater: number
			createNewSheet: (data?: TableData, options?: Partial<Exclude<Sheet, "data">>) => Sheet
			...
			setSheetCellSettingsConfig: React.Dispatch<React.SetStateAction<SheetCellSettingsConfig>>
			dispatch: (
				newState: Partial<LocalStoreType> | ((state: LocalStoreType) => Partial<LocalStoreType>)
			) => void
	  } & LocalStoreType &
			SheetStoreType &
