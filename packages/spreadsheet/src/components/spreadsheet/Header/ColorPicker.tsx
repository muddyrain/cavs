...
const ColorPickerInner: FC<{
	...
	const [recentColors, setRecentColorsState] = useState<ColorItemType[]>(getRecentColors())
	...
	const handleRecentColor = (item: ColorItemType) => {
		...
		colors = [item, ...colors.filter((c) => c.value !== item.value)].slice(0, 10)
			...
			content={
				<div className="w-74 flex flex-col pt-1">
					...
					>
						...
						<span className="text-sm ml-2">{backgroundType ? "无填充色" : "默认"}</span>
					</div>
					<ColorList
