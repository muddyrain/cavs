					...
					{sheets.map((item) => {
						return (
							<div
								...
								style={{
									color: activeSheetId === item.id ? config.selectionBorderColor : "black"
								}}
								onClick={() => {
							...
							>
								{item.name}
								{activeSheetId === item.id && <CheckIcon className="ml-auto" size={18} />}
							</div>
						)
					})}
				</PopoverContent>
										...
										createPortal(
											...
											>
												...
												<div
													...
													onClick={(e) => {
														e.stopPropagation()
														const textWidth = measureTextWidth(item.name, textRef.current)
