...
export const Menu: FC<{
	...
	const { handleCopy, handlePaste, handleCut, handlePasteText, handleClearContent } = useFunctions()
			...
			{list.map((item, key) => {
				...
				return (
					<div className="relative px-1 py-0.5 group" key={item.key || key}>
						...
						>
							<div className="w-6 h-6 flex items-center justify-center mr-1">{item.icon}</div>
							<span className="">{item.name}</span>
							{item.shortcutKey && (
								<span className="ml-auto text-zinc-400">{item.shortcutKey}</span>
							)}
							{item.children && (
