...
const useSheetStore = (sheet: SpreadsheetType) => {
	...
	const setHeaderColsWidth: SheetStoreActionType["setHeaderColsWidth"] = useCallback(
		(headerColsWidth) => {
			if (typeof headerColsWidth === "function") {
				headerColsWidth = headerColsWidth(currentSheet?.headerColsWidth || [])
			}
			setCurrentSheet("headerColsWidth", headerColsWidth)
		},
		[currentSheet, setCurrentSheet]
	)
	const setHeaderRowsHeight: SheetStoreActionType["setHeaderRowsHeight"] = useCallback(
		(headerRowsHeight) => {
			if (typeof headerRowsHeight === "function") {
				headerRowsHeight = headerRowsHeight(currentSheet?.headerRowsHeight || [])
			}
			setCurrentSheet("headerRowsHeight", headerRowsHeight)
		},
		[currentSheet, setCurrentSheet]
	)
	const setScrollPosition: SheetStoreActionType["setScrollPosition"] = useCallback(
		(scrollPosition) => {
			if (typeof scrollPosition === "function") {
				scrollPosition = scrollPosition(currentSheet?.scrollPosition || { x: 0, y: 0 })
			}
