...
export function useSheetSelection() {
	...
	const expandSelection = useCallback(
		(startRow: number, startCol: number, endRow: number, endCol: number) => {
			const checkMergedCells = (r1: number, c1: number, r2: number, c2: number) => {
					...
					for (let j = c1; j <= c2; j++) {
						...
						if (cell.mergeParent) {
							...
							if (parentCell.mergeSpan) {
								const newStartRow = Math.min(finalStartRow, parentCell.mergeSpan.r1)
								const newStartCol = Math.min(finalStartCol, parentCell.mergeSpan.c1)
								const newEndRow = Math.max(finalEndRow, parentCell.mergeSpan.r2)
								const newEndCol = Math.max(finalEndCol, parentCell.mergeSpan.c2)
	...
	const handleCellMouseDown = useCallback(
		(rowIndex: number, colIndex: number, wrapperRef: React.RefObject<HTMLDivElement | null>) => {
			...
			const handleMouseMove = (e: MouseEvent) => {
				...
				if (row !== lastRow || col !== lastCol) {
					const { newStartRow, newStartCol, newEndRow, newEndCol } = expandSelection(
						rowIndex,
						colIndex,
						row!,
						col!
