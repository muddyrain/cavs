...
export const useUpdateStyle = () => {
	...
	const isStyle = useMemo(() => {
		return {
			...
			isLineThrough:
				!!selectionCells?.length &&
				selectionCells.every((cell) => cell.style.textDecoration?.includes("line-through")),
			isUnderline:
				!!selectionCells?.length &&
				selectionCells.every((cell) => cell.style.textDecoration?.includes("underline")),
			isMergeCell: !!(currentCell && (currentCell.mergeSpan || currentCell.mergeParent)),
			isWrap: !!(currentCell && currentCell.style.wrap),
			isAlignLeft:
				!!selectionCells?.length && selectionCells.every((cell) => cell.style.textAlign === "left"),
			isALignCenter:
				!!selectionCells?.length &&
			...
			backgroundColor: !!selectionCells?.length && selectionCells[0]?.style.backgroundColor,
			color: !!selectionCells?.length && selectionCells[0]?.style.color,
			isUndo: deltaIndex >= 0,
			isRedo: deltaIndex < deltas.length - 1
		}
	}, [selectionCells, currentCell, formatBrushStyles.length, deltaIndex, deltas.length])
			...
			switch (type) {
				case "paint": {
					...
					for (let i = 0; i < rows; i++) {
						...
						for (let j = 0; j < cols; j++) {
							const cell = selectionCells.find((cell) => cell.row === r1 + i && cell.col === c1 + j)
				...
				case "strikethrough": {
					...
					newSelectionCells = produce(selectionCells, (draft) => {
						draft.forEach((cell) => {
							...
							if (isAll) {
								cell.style.textDecoration = cell.style.textDecoration?.replace("line-through", "")
							} else {
				...
				case "underline": {
					...
					newSelectionCells = produce(selectionCells, (draft) => {
						draft.forEach((cell) => {
							if (isAll) {
								cell.style.textDecoration = cell.style.textDecoration?.replace("underline", "")
							} else {
						...
						setData(
							produce((data) => {
								if (isAnchorMergePoint) {
									const target = data[selectedCell?.row || 0][selectedCell?.col || 0]
									target.mergeSpan = {
										r1,
										r2,
										c1,
			...
			setData(
				produce((data) => {
					...
					for (let i = 0; i < rows; i++) {
						for (let j = 0; j < cols; j++) {
							const style =
								formatBrushStyles[i % srcRows] && formatBrushStyles[i % srcRows][j % srcCols]
