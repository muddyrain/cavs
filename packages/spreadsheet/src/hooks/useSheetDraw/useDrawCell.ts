...
export const useDrawCell = () => {
	...
	const { renderCell, renderText, renderBorder, renderSelectedCell } = useRenderCell()
	const {
		getMergeCellSize,
		getCellPosition,
		getLeft,
	...
	const drawFrozenCols = useCallback(
		...
		[data, endRow, getCellPosition, renderBorder, renderCell, renderText, startRow]
	)
	// 绘制冻结首行（除左上角交叉单元格）
	const drawFrozenRows = useCallback(
		...
		[data, endCol, getCellPosition, renderBorder, renderCell, renderText, selection, startCol]
	)
	// 绘制左上角交叉单元格
