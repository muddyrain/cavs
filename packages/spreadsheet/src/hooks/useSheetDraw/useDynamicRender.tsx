...
export const useDynamicRender = () => {
	...
	const { getCellWidthHeight, getStartEndRow, getStartEndCol, getRenderAreaCells } = useComputed()
	...
	const getMeasureMap = useCallback(
		(ctx: CanvasRenderingContext2D) => {
			...
			for (let i = 0; i < cells.length; i++) {
				...
				const maxLengthContent = contents.reduce((a, b) => (a.length > b.length ? a : b), "")
			...
			if (temp && cell) {
				...
				if (!cell.readOnly && cell.value) {
					...
					if (textAlign === "left") {
						...
						const remainWidth = textWidth - (cellWidth - config.inputPadding * 2)
						let leftCol = col - 1
						let rightCol = col + 1
