...
export const useRenderCell = () => {
	...
	const { data, selection, config, headerColsWidth, headerRowsHeight } = useStore()
	...
	const renderText = useCallback(
		(ctx: CanvasRenderingContext2D, options: RenderOptions) => {
			...
			// 设置字体样式
			const { minWidth, color, textAlign, verticalAlign, fontSize } = getFontStyle(ctx, options)
			ctx.fillStyle = color
			...
			if (!cell.readOnly && textAlign === "left") {
				...
				let col = colIndex + 1
				// 向右累计宽度，直到遇到有内容的单元格
				while (col < data[rowIndex].length && !hasCellValue(data[rowIndex][col])) {
			...
			if (!cell.readOnly && textAlign === "center") {
				...
				while (rightCol < data[rowIndex].length && !hasCellValue(data[rowIndex][rightCol])) {
			...
			if (cell.readOnly) {
				...
				if (verticalAlign === "start") {
					...
					baseY = Math.max(
						y + cellHeight - totalTextHeight + lineHeightPX / 2 - config.inputPadding - 2,
						y + lineHeightPX / 2
					)
				}
	...
	const renderCell = useCallback(
		(ctx: CanvasRenderingContext2D, options: RenderOptions) => {
			...
			const cellHeight = height
			// 如果是非合并单元格
			const backgroundColor = cell.style.backgroundColor || config.backgroundColor
			// 校验当前
			if (!cell.mergeParent) {
				// 设置背景颜色
	...
	const renderBorder = useCallback(
		...
		[config.borderColor, drawBorderMap, getMergeCellSize, headerColsWidth, headerRowsHeight]
	)
	return {
		renderCell,
		renderText,
