...
export const useTools = () => {
	...
	const getFontSize = useCallback(
		(cell: CellData, isZoomSize: boolean = true) => {
			const fontSize = (cell.style.fontSize || config.fontSize) * (isZoomSize ? zoomSize : 1)
			return fontSize
		},
	...
	const getVerticalAlign = useCallback(
		(cell: CellData) => {
			return (cell.style.verticalAlign || config.verticalAlign) as CanvasTextBaseline
		},
	...
	const getFontStyle = useCallback(
		...
		[config.color, config.textAlign, config.verticalAlign, zoomSize, setFontStyle]
	)
	...
	const getWrapContent = useCallback(
		(ctx: CanvasRenderingContext2D, { cell, cellWidth }: { cell: CellData; cellWidth: number }) => {
			if (cell.style?.wrap) {
