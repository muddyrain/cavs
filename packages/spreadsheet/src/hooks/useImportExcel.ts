...
export function useImportExcel() {
	...
	const getColor = useCallback(
		(colorType: "background" | "text", appName: string | null, cell: ExcelJS.Cell): string => {
			...
			if (!returnColor)
				returnColor = colorType === "background" ? config.backgroundColor : config.color
			return returnColor
		},
		[config]
	)
				...
				const handler = async () => {
					try {
						...
						for (let i = 0; i < totalSheets; i++) {
							...
							const rows = (worksheet as unknown as { _rows: ExcelJS.Row[] })._rows
							const cols = (worksheet as unknown as { _columns: ExcelJS.Column[] })._columns
							...
							for (let i = 0; i < rowsTotal; i++) {
								...
								if (!row) {
									...
									row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
										...
										const backgroundColor = getColor("background", appName, cell)
										...
										rowData.push({
											...
											style: {
												...
												textDecoration: cell.font?.underline ? "underline" : "normal",
												textAlign: cell.alignment?.horizontal
													? (cell.alignment.horizontal as AlignType)
													: undefined,
												color: textColor,
												backgroundColor,
												borderColor: getSmartBorderColor(backgroundColor, config.borderColor),
									...
									while (indexNumber < colsTotal) {
										const address = generateColName(indexNumber) + (rowIndex + 1)
										rowData.push({
											value: "",
											style: {
												...getDefaultCellStyle()
									...
									if (rowsTotal > 0) {
										...
										options?.onProgress?.(sheetBase + sheetStep * (rowIndex / rowsTotal))
									}
								}
							}
							// 统计并设置 mergeSpan
							const mergeMap = new Map<string, { r1: number; r2: number; c1: number; c2: number }>()
