...
export const useFunctions = () => {
	...
	}, [data, endCol, endRow, selection, startCol, startRow, toHtmlTable, handleCopyText])
			...
			navigator.clipboard.read().then(async (items) => {
				...
				for (const item of items) {
					if (item.types.includes("text/html")) {
						...
						// 选区范围
						const selectionRangeString = getAttrFromHtml(html, DATA_SELECTION_RANGE)
								...
								produce((data) => {
									...
									for (let i = 0; i < tableData.length; i++) {
										for (let j = 0; j < tableData[i].length; j++) {
											if (!data[startRow + i] || !data[startRow + i][startCol + j]) continue
											...
											target.style = {
												...
												textAlign: (style.textAlign || config.textAlign) as AlignType,
												color: style.color || config.color,
												backgroundColor:
													style.background || style.backgroundColor || config.backgroundColor,
	...
	const handleCut = useCallback(() => {
		...
		if (selection.start && selection.end) {
			...
			const startCellAddress = generateColName(selection.start.col) + selection.start.row
			const endCellAddress = generateColName(selection.end.col) + selection.end.row
			...
			const tableString = toHtmlTable(data, startRow, endRow, startCol, endCol, tag)
			const text = handleCopyText()
			const clipboardItem = new ClipboardItem({
				"text/html": new Blob([tableString], { type: "text/html" }),
				"text/plain": new Blob([text], { type: "text/plain" })
