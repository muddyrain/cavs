...
export const useComputed = () => {
	...
	const updateSelectionAndCell = useCallback(
		(row: number, col: number) => {
			...
			if (cell.mergeSpan) {
				...
				const parentCell = getCurrentCell(cell.mergeParent.row, cell.mergeParent.col)
	...
	const getStartEndRow = useCallback(
		(wrapperHeight: number, _scrollPosition?: PositionType) => {
			...
			const viewBottom = (_scrollPosition || scrollPosition).y + wrapperHeight - headerRowsHeight[0]
	...
	const getStartEndCol = useCallback(
		(wrapperWidth: number, _scrollPosition?: PositionType) => {
			...
			const viewRight = (_scrollPosition || scrollPosition).x + wrapperWidth - headerColsWidth[0]
	...
	const getRealLeft = useCallback(
		(col: number) => {
			const beforeAllWidth =
				col === 0 ? 0 : headerColsWidth.slice(0, col).reduce((a, b) => a + b, 0) * zoomSize
	...
	const getLeft = useCallback(
		(col: number, _scrollPosition?: PositionType) => {
			return col === 0 ? 0 : getRealLeft(col) - (_scrollPosition || scrollPosition).x
		},
	...
	const getRealTop = useCallback(
		(row: number) => {
			const beforeAllHeight =
				row === 0 ? 0 : headerRowsHeight.slice(0, row).reduce((a, b) => a + b, 0) * zoomSize
	...
	const getTop = useCallback(
		(row: number, _scrollPosition?: PositionType) => {
			return row === 0 ? 0 : getRealTop(row) - (_scrollPosition || scrollPosition).y
		},
	...
	const getNextPosition = useCallback(
		(direction: ArrowDirectionType) => {
			...
			if (selectedCell && (selectedCell.mergeParent || selectedCell.mergeSpan)) {
	...
	// 绘制内容区（非冻结区）单元格
	// 添加缓存渲染区域的函数
	const getRenderAreaCells = useCallback(
