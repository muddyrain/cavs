...
export const useTab = () => {
	...
	const [alreadySelectedCell, setAlreadySelectedCell] = useState<MergeSpanType[]>([])
	const { getNextPosition, fitCellViewPort, updateSelectionAndCell } = useComputed()
	...
	const handleMultipleSelection = useCallback(() => {
		...
		if (selectedCell.mergeParent) {
			const cell = getCurrentCell(selectedCell.mergeParent.row, selectedCell.mergeParent.col)
			if (cell) {
				currentCell = cell
			}
		}
		...
		if (isMergeSpan) {
			...
			const currentCell = selectedCell.mergeParent
				? getCurrentCell(selectedCell.mergeParent.row, selectedCell.mergeParent.col)
				: selectedCell

			// 计算下一个列位置
			nextCol = currentCell?.mergeSpan?.c2 ? currentCell.mergeSpan.c2 + 1 : nextCol
			if (start && end) {
				// 处理换行逻辑
				if (nextCol > end.col) {
					...
					if (nextCell?.mergeParent) {
						nextCell = getCurrentCell(nextCell.mergeParent.row, nextCell.mergeParent.col)
					}
					// 处理合并单元格的跳过逻辑
					while (nextCell?.mergeSpan && isFindAlreadySelected(nextCell.mergeSpan)) {
