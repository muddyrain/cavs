...
export const useSheetScroll = (config: {
		...
		const onMouseMove = (e: MouseEvent) => {
			...
			if (dragType === "horizontal") {
				...
				const newScrollX = Math.max(
					0,
					Math.min(config.totalWidth - config.viewportWidth, lastScrollPos.x + scrollDelta)
				)
				...
				const newScrollY = Math.max(
					0,
					Math.min(config.totalHeight - config.viewportHeight, lastScrollPos.y + scrollDelta)
				)
	...
	}, [cellInputActions, config, selectedCell, setScrollPosition, getCurrentCell])
	...
	const handleWheel = useCallback(
		(e: WheelEvent) => {
			...
			const isInputInner = !!(role && ["cellInput", "cellInputInner"].includes(role))
			if (isInputInner) return
			let deltaX = e.deltaX
			let deltaY = e.deltaY
			// 兼容 Windows 下 shift+滚轮横向滚动
			if (e.shiftKey && Math.abs(deltaX) === 0 && deltaY !== 0) {
				deltaX = deltaY
				deltaY = 0
			}
			...
			// 只在需要横向滚动时处理横向滚动
			if (maxScrollX > 0) {
				newScrollX = Math.min(Math.max(0, scrollPosition.x + deltaX), maxScrollX)
			}
			// 只在需要纵向滚动时处理纵向滚动
			if (maxScrollY > 0) {
				newScrollY = Math.min(Math.max(0, scrollPosition.y + deltaY), maxScrollY)
			}
			// 只有当位置真正改变时才更新状态
