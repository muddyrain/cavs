...
export const TextAlignButton = (props: { textAlignment: TextAlignment }) => {
	...
	const textAlignment = useMemo(() => {
		...
		if (block.type === "table") {
			...
			const allCellsInTable = cellSelection.cells.map(
				({ row, col }) =>
					mapTableCell((block.content as TableContent<any, any>).rows[row].cells[col]).props
						.textAlignment
			)
		...
		(textAlignment: TextAlignment) => {
			...
			for (const block of selectedBlocks) {
				if (checkBlockTypeHasDefaultProp("textAlignment", block.type, editor)) {
					...
					const newTable = (block.content as TableContent<any, any>).rows.map((row) => {
						return {
							...row,
	...
	return (
		<Components.FormattingToolbar.Button
			className={"bn-button"}
			data-test={`alignText${
				props.textAlignment.slice(0, 1).toUpperCase() + props.textAlignment.slice(1)
			}`}
