...
export const Comments = ({ thread, maxCommentsBeforeCollapse }: CommentsProps) => {
	...
	const comments = thread.comments.map((comment, index) => (
		<Comment key={comment.id} thread={thread} comment={comment} showResolveButton={index === 0} />
	))
	...
	if (thread.resolved && thread.resolvedUpdatedAt && thread.resolvedBy) {
		...
		const resolvedCommentIndex =
			thread.comments.findLastIndex(
				(comment) => thread.resolvedUpdatedAt!.getTime() > comment.createdAt.getTime()
			) + 1
		...
		comments.splice(
			...
			<Components.Comments.Comment
				...
				authorInfo={(thread.resolvedBy && users.get(thread.resolvedBy)) || "loading"}
				...
				showActions={false}
			>
				<div className={"bn-resolved-text"}>{dict.comments.sidebar.marked_as_resolved}</div>
			</Components.Comments.Comment>
		)
	}

	// Collapses replies if needed.
	if (maxCommentsBeforeCollapse && comments.length > maxCommentsBeforeCollapse) {
		comments.splice(
			1,
			comments.length - 2,
