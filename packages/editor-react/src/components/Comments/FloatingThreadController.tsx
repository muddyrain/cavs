...
import { ComponentProps, FC, useCallback, useEffect, useLayoutEffect } from "react"
...
>(props: {
	...
	const { isMounted, ref, style, getFloatingProps, setReference } = useUIElementPositioning(
	...
	const updateRef = useCallback(() => {
		...
		const el = editor.domElement?.querySelector(`[data-bn-thread-id="${state?.selectedThreadId}"]`)
		if (el) {
			setReference(el)
		}
	...
	return (
		<div ref={ref} style={style} {...getFloatingProps()}>
			<Component thread={threads.get(state.selectedThreadId)!} selected={true} />
		</div>
	)
}
