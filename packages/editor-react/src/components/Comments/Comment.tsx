...
import type { CommentData, ThreadData } from "@cavs/editor-core/comments"
 ...
 *
 */
export const Comment = ({ comment, thread, showResolveButton }: CommentProps) => {
	...
	if (!isEditing) {
		actions = (
			...
			>
				{canAddReaction && (
					<EmojiPicker
						onEmojiSelect={(emoji: { native: string }) => onReactionSelect(emoji.native)}
					>
								...
								<>
									{comment.reactions.length > 0 && !isEditing && (
										<Components.Generic.Badge.Group
											className={mergeCSSClasses("bn-badge-group", "bn-comment-reactions")}
										>
											...
											>
												<Components.Generic.Badge.Root
													className={mergeCSSClasses("bn-badge", "bn-comment-add-reaction")}
													text={"+"}
