...
>(
			...
			content: {
				...
				rows:
					props.orientation === "addOrRemoveColumns"
						? props.editor.tableHandles!.addRowsOrColumns(props.block, "columns", 1)
						: props.editor.tableHandles!.addRowsOrColumns(props.block, "rows", 1)
			} as any
		})
	...
	useEffect(() => {
		const callback = (event: MouseEvent) => {
			...
			const diff =
				(props.orientation === "addOrRemoveColumns" ? event.clientX : event.clientY) -
			...
			const newNumCells =
				...
				marginRound(
					diff /
						(props.orientation === "addOrRemoveColumns" ? EMPTY_CELL_WIDTH : EMPTY_CELL_HEIGHT),
					0.3
				)
