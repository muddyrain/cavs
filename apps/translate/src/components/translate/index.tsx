...
export const Translate: FC<{
  ...
  const startTranslate = useCallback(async () => {
    try {
      ...
      setLoading(true)
      const res = await fetch(`https://cavs-api.vercel.app/api/translate/baidu?q=${inputText}&from=${originLang}&to=${targetLang}`)
    ...
    <div className="bg-slate-700 border border-solid border-slate-500 text-white rounded-md shadow-lg w-[400px]">
      ...
      <div className="flex items-center justify-between p-2">
        ...
        <div className="flex items-center gap-2">
          {/* 图标/下拉框等 */}
          {
            isShowClose && <button className="text-white hover:text-slate-300" onClick={() => {
              if (onClose) onClose()
            }}>
              <X />
            </button>
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-3 mt-3">
          ...
          <button className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 text-sm" onClick={startTranslate}>
            翻译
          </button>
        </div>
        {/* 结果区 */}
        {
          isResultVisible && (<div className="bg-slate-600 mt-3 rounded text-sm relative overflow-hidden">
            {loading ? (
              ...
              <>
                {
                  result.length > 0 ? <div className="p-2 break-words">
                    {result}
                  </div> : null
                }
              </>
